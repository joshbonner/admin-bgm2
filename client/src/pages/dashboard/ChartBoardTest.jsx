...
import { getRoasData, getRoasMultiData, insertRoasData } from "../../api/chart";
...
export default function ChartBoard(props) {
  ...
  const chartLoad = async () => {
    ...
    // Get ROAS Data for each connection and date
    storedRoas = await getRoasMultiData(
      chartDate.start,
      chartDate.end,
      searchedVal.plugAccount,
      searchedVal.adAccount.map((item) => item.name),
    );
    storedRoas = storedRoas.map((item) => ({ ...item, isStored: true }));
    console.log(storedRoas);

    // Get Date Duration
    var dateDuration = 0;
    while (true) {
      dateDuration++;
      const currentDate = dayjs(chartDate.start)
        .add(dateDuration - 1, "days")
        .format("YYYY-MM-DD");
      if (currentDate === chartDate.end) break;
    }

    var i = 1;
    var unstoredRoas = [];
    while (true) {
      const curDate = dayjs(chartDate.start)
        .add(i - 1, "days")
        .format("YYYY-MM-DD");
      for (const plug of searchedVal.plugAccount) {
        for (const ad of searchedVal.adAccount) {
          if (
            storedRoas.filter(
              (item) =>
                item.adAccount === ad &&
                item.plugAccount === plug &&
                curDate === item.date,
            ).length === 0
          ) {
            unstoredRoas = [
      ...
      i++;
      if (curDate === chartDate.end) break;
    }

    var newRevenues = [];
    for (i = 0; i < unstoredRoas.length; i++) {
      const connectionByDay = await getDataByConnection(
        unstoredRoas[i].date,
        unstoredRoas[i].date,
        unstoredRoas[i].data.plugAccount,
        unstoredRoas[i].data.adAccount,
        searchedVal.defaultTimezone,
      );
      newRevenues = [
        ...newRevenues,
        {
          date: unstoredRoas[i].date,
          data: connectionByDay,
        },
      ];
    }
    ...
    for (i = 0; i < dateDuration; i++) {
      const currentDate = dayjs(chartDate.start)
        .add(dateDuration - i, "days")
        .format("YYYY-MM-DD");
      var roasByDay = [];

      if (storedRoas.filter((item) => item.date === currentDate).length !== 0) {
        roasByDay = [
          ...roasByDay,
          ...storedRoas.filter((item) => item.date === currentDate)[0].data,
        ];
      }
      if (
        newRevenues.filter((item) => item.date === currentDate).length !== 0
      ) {
        roasByDay = [
          ...roasByDay,
          newRevenues
            .filter((item) => item.date === currentDate)[0]
            .data.map((item) => ({
              adAccount: item.adAccount,
              plugAccount: item.plugAccount,
              date: currentDate,
            })),
        ];
      }
    ...
    while (true) {
      const curDate = dayjs(chartDate.start)
        .add(i - 1, "days")
        .format("YYYY-MM-DD");
      if (storedRoas.filter((item) => item.date === curDate).length === 0) {
        ...
        chartResult.push({
          roas: storedRoas.filter((item) => item.date === curDate)[0].roas,
          isStored: true,
          date: curDate,
        });
      }
