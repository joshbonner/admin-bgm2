...
export default function ConnectedList(props) {
  ...
  React.useEffect(() => {
    if (!isEmpty(props.data)) setState({ data: props.data });
  }, [props.data]);
    ...
    <StyledList>
      {props.isLoading ? (
        ...
        state.data.map((item) => (
          ...
          >
            <div
              style={{ width: "45%", display: "flex", flexDirection: "row" }}
            >
              {!isEmpty(item.icon) && (
                <img
                  style={{ borderRadius: "50%", margin: "15px 10px" }}
                  width={25}
                  height={25}
                  src={item.icon}
                />
            ...
            <p style={{ width: "45%", textAlign: "right" }}>
              {item.adgroupName}
            </p>
          </StyledListItem>
        ))
