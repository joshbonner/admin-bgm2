...
import { StyledTableCell, StyledTableContainer } from "./tableStyles";
                  ...
                  <TableRow key={item.key} hover>
                    {props.columns.map((col) => (
                      <StyledTableCell
                        ...
                        key={`${item.key + index++}`}
                      >
                        {item.edit && col.editable ? (
                          <StyledTextField name={col.id} value={item[col.id]} />
                        ) : isEmpty(col.render) ? (
                          item[col.id]
                        ) : (
                          col.render(item[col.id], item)
                        )}
                      </StyledTableCell>
                    ))}
                  </TableRow>
            ...
            {props.total && !isEmpty(props.data) && (
              <TableRow style={{ backgroundColor: "#f2f2f2" }}>
                ...
                <StyledTableCell
                  style={{ color: "#333" }}
                  align="center"
                >{`$ ${props.totalRow.revenue}`}</StyledTableCell>
                <StyledTableCell
                  style={{ color: "#333" }}
                  align="center"
                >{`$ ${props.totalRow.spend}`}</StyledTableCell>
                <StyledTableCell
                  align="center"
                  style={
                ...
                <StyledTableCell style={{ color: "#333" }} align="center">{`${(
                  Number(props.totalRow.roas) * 100
