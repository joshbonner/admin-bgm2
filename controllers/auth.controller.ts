...
import VisitModel, { IVisit } from "../models/Visits";
...
export const getIp = asyncHandler(async (req: IUserRequest, res: Response) => {
  try {
    new VisitModel({ ipaddress: req.body.ip, date: req.body.date })
      .save()
      .then((item) => {
        console.log(item);
      });
...
export const getVisits = asyncHandler(
  async (req: IUserRequest, res: Response) => {
    try {
      VisitModel.find().then((item) => res.json(item));
    } catch (err: any) {
      console.error(err.message);
      res.status(500).send("Server Error");
    }
