...
export const getData: RequestHandler = (req, res) => {
  SnapSetModel.find().then((accounts: Array<ISnapSet>) =>
...
export const addData: RequestHandler = (req, res) => {
  ...
  newSnapset.save().then((account: ISnapSet) => res.json(account));
};
...
export const editData: RequestHandler = (req, res) => {
  SnapSetModel.findByIdAndUpdate(
    req.params._id,
    { $set: req.body },
    { new: true },
...
export const deleteData: RequestHandler = (req, res) => {
  SnapSetModel.findByIdAndDelete(req.params._id).then((item) => res.json(item));
};
...
export const deleteMany: RequestHandler = (req, res) => {
  SnapSetModel.deleteMany(req.body).then((deletes) => res.json(deletes));
};
